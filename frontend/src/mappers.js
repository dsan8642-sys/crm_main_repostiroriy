...
export function mapClientPortalData({ overview, profile, consents, schedule, attendance, payments, notifications, resourceStates, resolvedStudentId: requestedStudentId }) {
  ...
  const children = participants.length
    ? participants.map((student) => ({
      ...
      nextSession: student.next_session ? {
        sessionId: student.next_session.id,
        date: formatShortDate(student.next_session.start_at),
        start: formatTime(student.next_session.start_at),
        end: formatTime(student.next_session.end_at),
        group: student.next_session.group?.name || mapperLabel('individualLesson'),
        trainer: student.next_session.trainer || '',
        location: student.next_session.location || '',
        status: sessionStatus(student.next_session),
      } : null,
...
export function mapAdminPortalData({ reference, clients, trainers, groups, subscriptionTypes, sessionTypeConfigs, sessions, payments, debtors }) {
  ...
  return {
    ...
    sessions: (sessions.sessions || []).map((session) => ({
      ...
      group: session.group?.name || mapperLabel('individualLesson'),
      trainer: session.trainer,
      trainerArchived: session.trainer_is_active === false,
      groupArchived: session.group_is_active === false,
      trainerArchived: session.trainer_is_active === false,
      groupArchived: session.group_is_active === false,
      location: session.location,
      count: session.participants_count || 0,
