...
$operatorChecklist = @(
    ...
    [ordered]@{
        ...
        expected_evidence = "docs\PRODUCTION_CUTOVER_EVIDENCE.json contains only real production evidence for commit $commitSha and archive SHA256 $($releaseArchive.archive_sha256), including Release source archive contents verified and complete rollback acknowledgement: Rollback plan reviewed; stop writers; verified backup; restore; migrate --check; restart services; live smoke."
        stop_if_missing = $true
    },
    [ordered]@{
