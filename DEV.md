...
└─ frontend/    # React + Vite SPA (пока страница-заглушка; сюда лягут порталы клиента/тренера)
```
...
- Запросы к `/api/*` Vite сам проксирует на Django `:8000` (см. `frontend/vite.config.js`),
  поэтому CORS настраивать не нужно. **Backend должен быть запущен**, иначе заглушка
